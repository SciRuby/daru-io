RSpec.describe Daru::IO::Importers::RDS do
  subject { described_class.new(path, index: index).call }
  ...
  context 'reads data from RDS file with index' do
    let(:path)  { 'spec/fixtures/rds/bc_sites.rds' }
    let(:index) { 'location_code' }

    it_behaves_like 'exact daru dataframe',
      ncols: 24,
      nrows: 1113,
      order: %w[
        area description epa_reach format_version latitude location location_type longitude
        name psc_basin psc_region record_code record_origin region reporting_agency rmis_basin
        rmis_latitude rmis_longitude rmis_region sector state_or_province sub_location
        submission_date water_type
      ]
  end
end
